...
export class RequisitionProvider {
  ...
  constructor(public http: HttpClient) {
    console.log('Hello RequisitionProvider Provider');
  }
  ...
  issueParts(itemReqId, processedBy, quantity, location) {
    this.http.post('http://localhost:64778/api/requisition/issue', {itemReqId: itemReqId, processedBy: processedBy, quantity: quantity, location: location}).subscribe(response => {
